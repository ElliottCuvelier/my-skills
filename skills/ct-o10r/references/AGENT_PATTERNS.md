...
This is the skill's **signal → teammate-role recipe** library. When `analyze_codebase.py`
detects a signal, Claude reads this table to decide which teammate roles to propose.
...
Each role is reused as an agent-team teammate role: the lead spawns a teammate of
that type to own the matching files (`scope_hint`) as its exclusive lane.
...
todos that don't match any project role's scope_hint.

| Role | Model | Purpose |
|-------|-------|---------|
| `impl-haiku` | haiku | Generic cheap implementer teammate — bulk changes, typo fixes, simple refactors |
| `impl-sonnet` | sonnet | Generic balanced implementer teammate — default for most implementation work |
| `impl-opus` | opus | Generic premium implementer teammate — complex logic, escape hatch |
| `impl-inherit` | inherit | Same model as the lead — for lanes needing full planning-tier reasoning |
| `verifier` | haiku | Read-only reviewer teammate (used when a plan sets `team.verify: review`) |
| `memory-curator` | inherit | End-of-run consolidation teammate |
