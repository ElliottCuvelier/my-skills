...
verbatim into every generated teammate role when ByteRover is enabled.
...
**Rule**: Teammates use `brv search` only. `brv query` (LLM-synthesized answer) is reserved for
the lead because it costs an LLM call.
...
- **Blocking** — never `--detach` as a teammate (context dies on return).
- **Max 5 `-f` files**, project-relative paths only.
- **Skip when nothing new** — "skipped — nothing new" beats low-signal curates.
- **Never auto-approve** — report `taskId`s and message the lead `BRV-REVIEW <taskId>`; the lead surfaces them to the user.
