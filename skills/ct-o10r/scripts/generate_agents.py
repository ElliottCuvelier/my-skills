#!/usr/bin/env python3
"""
Render the ct-o10r (claudeteam-orchestrator) templates into teammate-role and
command files at the target scope(s). Idempotent: existing files whose SHA
matches the stored marker hash are overwritten; user-edited files (hash
mismatch) are left alone and reported as warnings; pre-existing untracked files
are registered and never touched.

Usage:
    # Generate from scratch using answers JSON
    python scripts/generate_agents.py --answers <json>

    # Regenerate using the existing marker (for /update-ct-orchestrator)
...
def _plan_file_list(
    ...
    # impl-* baseline teammate roles
    impl_tmpl = _read_template("teammate-role.md.tmpl")
    ...
    if answers["commands"]:
        for cmd_tmpl_name, cmd_filename in [
            ("orchestrate-cmd.md.tmpl", "ct-orchestrate.md"),
            ("orchestrate-resume-cmd.md.tmpl", "ct-orchestrate-resume.md"),
            ("update-cmd.md.tmpl", "update-ct-orchestrator.md"),
        ]:
...
# Maps hook id → list of (template filename, generated script filename, event,
# matcher|None, timeout ms). One selection can install several team hooks: the
# "tests" gate needs the TaskCreated sidecar (which records each task's files)
# alongside the TaskCompleted gate that reads it back.
_HOOK_SPEC: dict[str, list[tuple[str, str, str, str | None, int]]] = {
    "tests": [
        ("hooks/taskcreated-files-sidecar.py.tmpl", "taskcreated-files-sidecar.py", "TaskCreated", None, 5_000),
        ("hooks/run-task-tests.py.tmpl", "run-task-tests.py", "TaskCompleted", None, 120_000),
    ],
    "taskids": [
        ("hooks/collect-taskids.py.tmpl", "collect-taskids.py", "TeammateIdle", None, 10_000),
    ],
}
...
def _install_hooks(
    ...
    for hook_id in hooks_to_install:
        if hook_id not in _HOOK_SPEC:
            continue
        for tmpl_name, script_name, event, matcher, timeout_ms in _HOOK_SPEC[hook_id]:
